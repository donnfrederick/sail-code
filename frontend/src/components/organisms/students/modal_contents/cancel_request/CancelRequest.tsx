...
interface Props {
  ...
  close(): void
  deleteRequestConversations(authToken: string, id: number): Promise<void>
  open(): void
...
export default (props: Props) => {
  ...
  return (
    <Container>
      <ModalTextContainer heading={'Do you want to cancel the request?'} />
      <ButtonContainer>
        <Button
          type="blue"
          text="No"
        ...
        <Button
          ...
          onClick={async () => {
            if (conversation) {
              try {
                await deleteRequestConversations(authToken, conversation.id)
                close()
